...
public class AdminDAO {
	...
	public AdminDTO getAdminInfo(String id) {
		AdminDTO dto = null;
		String sql = "SELECT * FROM flower_admin WHERE id = ?";
	...
	// 관리자 로그인 체크 
	public int adminOk(String id, String pw) {
		int n=0;
		String dbPw;
		
		String sql = "SELECT password FROM flower_admin WHERE id = ?";
		conn = getConnection();
		...
		try {
			...
			if(rs.next()) {
				...
				if(dbPw.equals(pw)) { // 비밀번호 일치 로그인 성공
