...
public class CategoryDAO {
	...
	public int insertCategory(String code, String cname) {
		int n = -1;
		String sql = "INSERT INTO flower_category VALUES(f_cat_seq.nextVal,?,?)";
		conn = getConnection();
	...
	public ArrayList<CategoryDTO> categoryList(){
		...
		String sql = "SELECT * FROM flower_category ORDER BY cat_num DESC";
		conn = getConnection();
	...
	public int categoryDel(String catNum) {
		int n = -1;
		String sql = "DELETE FROM flower_category WHERE cat_num = ?";
		conn = getConnection();
		try {
			ps = conn.prepareStatement(sql);
