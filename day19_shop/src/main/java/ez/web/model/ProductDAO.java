...
public class ProductDAO {
	...
	public ArrayList<ProductDTO> getProductByCategory(String code){
		...
		try {
			...
			// 해시맵 추가하기 : 카테고리 코드별로 각각 상품List가 맵에 추가된다.
			map.put(code, pDtos);
		}catch(Exception e) {
			e.printStackTrace();
		}finally {
	...
	// 사용자가 장바구니에 담고자하는 상품 가져오기위한 메소드 
		public ProductDTO getProduct(String pNum, String pSpec) {
			...
			System.out.println("pNum:" +pSpec);
			
			// 해시맵에서 사양별 상품 리스트 가져오기 
			ArrayList<ProductDTO> pDtos = map.get(pSpec);
			...
			// 사양별 상품리스트에서 사용자가 담고자 하는 상품번호와 일치하는 상품 가져오기
